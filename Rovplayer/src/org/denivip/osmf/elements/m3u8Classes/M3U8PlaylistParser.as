			...
			for(var i:int = 1; i < lines.length; i++){
				...
				if(line.indexOf("#EXT-X-STREAM-INF:") == 0){
					if(!result){
						...
						var temp_res2:StreamingURLResource = (baseResource as StreamingURLResource);
						if(!temp_res2)
							temp_res2 = new StreamingURLResource(baseResource.url);
						
						tempStreamingRes = HLSStreamingResource.createHLSResource(temp_res2);
						if(tempStreamingRes){
